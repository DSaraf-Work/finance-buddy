...
export default function KeywordTagInput({ 
  ...
  const getUsageBadgeColor = (category: string) => {
    switch (category) {
      ...
      case 'rare':
        return 'bg-airbnb-gray-light/50 border-airbnb-border-light';
      default:
        return 'bg-airbnb-gray-light/50 border-airbnb-border-light';
    }
  };
  ...
  return (
    <div className={`relative ${className}`}>
      <div className="w-full min-h-[42px] px-3 py-2 border border-airbnb-border-light rounded-airbnb-md focus-within:ring-2 focus-within:ring-blue-500 focus-within:border-blue-500 transition-colors">
      ...
      {showSuggestions && filteredSuggestions.length > 0 && (
        <div className="absolute z-10 w-full mt-1 bg-airbnb-white border border-airbnb-border-light rounded-airbnb-md shadow-lg max-h-60 overflow-y-auto">
          {filteredSuggestions.map((keyword) => (
            <button
              key={keyword.id}
              type="button"
              onClick={() => handleSuggestionClick(keyword)}
              className={`w-full px-3 py-2 text-left hover:bg-airbnb-gray-light/50 focus:bg-airbnb-gray-light/50 focus:outline-none border-l-2 ${getUsageBadgeColor(keyword.usage_category)}`}
            >
            ...
            {keywords
              ...
              .map((keyword) => (
                <button
                  ...
                  className="px-2 py-1 text-xs bg-airbnb-gray-light/30 text-airbnb-text-secondary rounded-full hover:bg-gray-200 focus:outline-none focus:ring-1 focus:ring-blue-500"
                >
                  {keyword.keyword}
                </button>
              ))}
