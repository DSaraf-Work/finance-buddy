// Perplexity AI Model Implementation
// Perplexity API is OpenAI SDK compatible
...
export class PerplexityModel extends BaseAIModel {
  ...
  async generateResponse(request: AIRequest): Promise<AIResponse> {
    try {
      ...
      const completion = await this.client.chat.completions.create({
        ...
        // Note: Perplexity doesn't support response_format for JSON mode
        // It will return text responses
      });
  ...
  async isHealthy(): Promise<boolean> {
    try {
      ...
      console.error(`Perplexity health check failed for ${this.config.model}:`, error);
      return false;
    }
  }
  ...
  async getRateLimitStatus(): Promise<{ remaining: number; resetTime: Date }> {
    ...
    const minuteRemaining = Math.max(0, 
      (this.config.rateLimit?.requestsPerMinute || 50) - this.getMinuteRequestCount()
    );
    
    const hourlyRemaining = Math.max(0,
      (this.config.rateLimit?.requestsPerHour || 5000) - this.hourlyRequestCount
    );
