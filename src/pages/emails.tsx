      ...
      >
        <div className="py-6">
          ...
          <div className="bg-[var(--color-bg-card)] rounded-[var(--radius-lg)] shadow-[var(--shadow-lg)] border border-[var(--color-border)] p-3 mb-4">
            ...
            {/* Row 1: Date range - always inline */}
            <div className="flex items-center gap-1 mb-2">
              ...
              <input
                ...
                className="input-field text-[10px] py-0.5 px-1 flex-1 min-w-0"
            ...
            {/* Row 2: Status, Page, Size - always inline */}
            <div className="flex items-center gap-1 mb-2">
              <select
                value={filters.status || ''}
                onChange={(e) => handleFilterChange('status', e.target.value)}
                className="input-field text-[10px] py-0.5 px-1 flex-1"
                style={{ maxWidth: '80px' }}
              >
              ...
              <input
                ...
                className="input-field text-[10px] py-0.5 px-1 w-10"
              />
            ...
            <div className="flex flex-wrap gap-2 mb-2">
              ...
              <Collapsible open={accountsExpanded} onOpenChange={setAccountsExpanded} className="flex-1 min-w-[200px]">
                <div className="border border-[var(--color-border)] rounded">
                  <CollapsibleTrigger className="flex items-center justify-between w-full px-2 py-1 hover:bg-[var(--color-bg-elevated)] rounded transition-colors">
                    <div className="flex items-center gap-1.5">
                      <span className="text-xs font-medium text-[var(--color-text-secondary)]">
                        Accounts<span className="text-red-500">*</span>
                      </span>
                      <span className="text-[10px] bg-[var(--color-accent-primary)]/20 text-[var(--color-accent-primary)] px-1 py-0.5 rounded">
                        {filters.email_addresses?.length || 0}/{connections.length}
                      </span>
                    </div>
                    <ChevronDown className={`h-3 w-3 text-[var(--color-text-muted)] transition-transform ${accountsExpanded ? 'rotate-180' : ''}`} />
                  </CollapsibleTrigger>
                  <CollapsibleContent className="px-2 pb-1.5">
                    {loadingConnections ? (
                      <div className="text-[10px] text-[var(--color-text-muted)] py-1">Loading...</div>
                    ) : connections.length === 0 ? (
                      <div className="text-[10px] text-[var(--color-expense)] py-1">No connections</div>
                    ) : (
                      <>
                        <div className="flex gap-2 mb-1 pt-0.5">
                          <button type="button" onClick={() => handleSelectAllAccounts(true)} className="text-[10px] text-[var(--color-accent-primary)] hover:underline">All</button>
                          <button type="button" onClick={() => handleSelectAllAccounts(false)} className="text-[10px] text-[var(--color-text-muted)] hover:underline">Clear</button>
                        </div>
                        <div className="space-y-0.5 max-h-16 overflow-y-auto">
                          {connections.map((connection) => (
                            <label key={connection.id} className="flex items-center gap-1.5 cursor-pointer">
                              <Checkbox checked={filters.email_addresses?.includes(connection.email_address) || false} onCheckedChange={(checked) => handleEmailAddressChange(connection.email_address, checked as boolean)} className="h-3 w-3" />
                              <span className="text-[10px] text-[var(--color-text-secondary)] truncate">{connection.email_address}</span>
                            </label>
                          ))}
                        </div>
                      </>
              ...
              {/* Senders - Collapsible */}
              <Collapsible open={sendersExpanded} onOpenChange={setSendersExpanded} className="flex-1 min-w-[200px]">
                <div className="border border-[var(--color-border)] rounded">
                  <CollapsibleTrigger className="flex items-center justify-between w-full px-2 py-1 hover:bg-[var(--color-bg-elevated)] rounded transition-colors">
                    <div className="flex items-center gap-1.5">
                    ...
                    <ChevronDown className={`h-3 w-3 text-[var(--color-text-muted)] transition-transform ${sendersExpanded ? 'rotate-180' : ''}`} />
                  </CollapsibleTrigger>
                  <CollapsibleContent className="px-2 pb-1.5">
                    {bankAccountTypes.length === 0 ? (
                      <div className="text-[10px] text-[var(--color-text-muted)] py-1">No senders</div>
                    ) : (
                      <>
                        <div className="flex gap-2 mb-1 pt-0.5">
                          <button type="button" onClick={() => handleSelectAllSenders(true)} className="text-[10px] text-[var(--color-accent-primary)] hover:underline">All</button>
                          <button type="button" onClick={() => handleSelectAllSenders(false)} className="text-[10px] text-[var(--color-text-muted)] hover:underline">Clear</button>
                        </div>
                        <div className="space-y-0.5 max-h-16 overflow-y-auto">
                          {bankAccountTypes.map((sender) => (
                            <label key={sender} className="flex items-center gap-1.5 cursor-pointer">
                              <Checkbox checked={filters.senders?.includes(sender) || false} onCheckedChange={(checked) => handleSenderChange(sender, checked as boolean)} className="h-3 w-3" />
                              <span className="text-[10px] text-[var(--color-text-secondary)] truncate">{sender}</span>
                            </label>
                          ))}
                        </div>
                      </>
            ...
            <div className="flex flex-wrap items-center gap-1.5">
              <label className="flex items-center gap-1">
                ...
                <span className="text-[10px] text-[var(--color-text-secondary)]">DB Only</span>
              </label>

              <Button onClick={handleSearch} disabled={loading} size="sm" className="h-6 px-2 text-xs">
                {loading ? '...' : 'Search'}
              </Button>
              ...
              <Button onClick={handleGetAllFetched} disabled={batchProcessing || loading} variant="outline" size="sm"
                className="h-6 px-2 text-xs border-[var(--color-income)] text-[var(--color-income)] hover:bg-[var(--color-income)]/10">
                {batchProcessing && batchProgress.total > 0 ? `${batchProgress.current}/${batchProgress.total}` : 'Get Fetched'}
              </Button>

              <Button onClick={handleBulkProcess} disabled={batchProcessing || loading} variant="outline" size="sm"
                className="h-6 px-2 text-xs border-[var(--color-accent-primary)] text-[var(--color-accent-primary)] hover:bg-[var(--color-accent-primary)]/10">
                {batchProcessing && batchProgress.total > 0 && batchProgress.current <= emails.length ? `${batchProgress.current}/${batchProgress.total}` : 'Bulk Process'}
              </Button>
            </div>
          </div>
