...
const TransactionWorkbenchPage: NextPage = () => {
  ...
  const getConfidenceColor = (confidence?: number) => {
    ...
    if (confidence >= 0.8) return 'bg-accent-emerald/10 text-accent-emerald';
      ...
      >
        <div className="py-6">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            {/* Header */}
            <div className="mb-6">
              <h1 className="text-2xl font-bold text-text-primary">Transaction Workbench</h1>
            ...
            {/* Filters */}
            <div className="bg-bg-secondary shadow rounded-lg mb-6">
              ...
              <div className="px-6 py-4">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-text-secondary mb-1">Merchant</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-sm font-medium text-text-secondary mb-1">Min Amount</label>
                    <input
                      type="number"
                      step="0.01"
                  ...
                  <div>
                    <label className="block text-sm font-medium text-text-secondary mb-1">Max Amount</label>
                    <input
                      type="number"
                      step="0.01"
                  ...
                  <div>
                    <label className="block text-sm font-medium text-text-secondary mb-1">Min Confidence</label>
                    <input
                      type="number"
                      step="0.1"
                      min="0"
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
                  <div>
                    <label className="block text-sm font-medium text-text-secondary mb-1">Page Number</label>
                    <input
                      type="number"
                      min="1"
                  ...
                  <div>
                    <label className="block text-sm font-medium text-text-secondary mb-1">Page Size</label>
                    <input
                      type="number"
                      min="1"
                      max="100"
            ...
            <div className="bg-bg-secondary shadow rounded-lg overflow-hidden">
              ...
              {pagination.totalPages > 1 && (
                <div className="bg-bg-secondary px-4 py-3 border-t border-border sm:px-6">
                  <div className="flex items-center justify-between">
        ...
        {showDrawer && selectedTransaction && (
          <div className="fixed inset-0 z-50 overflow-hidden">
            ...
            <div className="absolute right-0 top-0 h-full w-full max-w-2xl bg-bg-secondary shadow-xl">
              <div className="flex flex-col h-full">
                ...
                <div className="flex-1 overflow-y-auto p-6">
                  <div className="space-y-6">
                    <div>
                      <h3 className="text-sm font-medium text-text-primary mb-2">Transaction Information</h3>
                      <dl className="grid grid-cols-1 gap-x-4 gap-y-3 sm:grid-cols-2">
                        <div>
                          <dt className="text-sm font-medium text-gray-500">Transaction Time</dt>
                          <dd className="text-sm text-text-primary">{formatDate(selectedTransaction.txn_time)}</dd>
                        </div>
                        <div>
                        ...
                        <div>
                          ...
                          <dd className="text-sm text-text-primary">{selectedTransaction.currency || 'N/A'}</dd>
                        </div>
                        <div>
                          <dt className="text-sm font-medium text-gray-500">Merchant Name</dt>
                          <dd className="text-sm text-text-primary">{selectedTransaction.merchant_name || 'N/A'}</dd>
                        </div>
                        <div>
                          <dt className="text-sm font-medium text-gray-500">Merchant (Normalized)</dt>
                          <dd className="text-sm text-text-primary">{selectedTransaction.merchant_normalized || 'N/A'}</dd>
                        </div>
                        <div>
                          <dt className="text-sm font-medium text-gray-500">Category</dt>
                          <dd className="text-sm text-text-primary">{selectedTransaction.category || 'N/A'}</dd>
                        </div>
                        <div>
                          <dt className="text-sm font-medium text-gray-500">Account Hint</dt>
                          <dd className="text-sm text-text-primary">{selectedTransaction.account_hint || 'N/A'}</dd>
                        </div>
                        <div>
                          <dt className="text-sm font-medium text-gray-500">Reference ID</dt>
                          <dd className="text-sm text-text-primary font-mono">{selectedTransaction.reference_id || 'N/A'}</dd>
                        </div>
                        <div>
                          <dt className="text-sm font-medium text-gray-500">Location</dt>
                          <dd className="text-sm text-text-primary">{selectedTransaction.location || 'N/A'}</dd>
                        </div>
                        <div>
                        ...
                        <div>
                          ...
                          <dd className="text-sm text-text-primary">{selectedTransaction.extraction_version || 'N/A'}</dd>
                        </div>
                      </dl>
                    </div>

                    <div>
                      <h3 className="text-sm font-medium text-text-primary mb-2">Metadata</h3>
                      <dl className="grid grid-cols-1 gap-x-4 gap-y-3">
                        <div>
                          <dt className="text-sm font-medium text-gray-500">Email Row ID</dt>
                          <dd className="text-sm text-text-primary font-mono">{selectedTransaction.email_row_id}</dd>
                        </div>
                        <div>
                          <dt className="text-sm font-medium text-gray-500">Google User ID</dt>
                          <dd className="text-sm text-text-primary font-mono">{selectedTransaction.google_user_id}</dd>
                        </div>
                        <div>
                          <dt className="text-sm font-medium text-gray-500">Connection ID</dt>
                          <dd className="text-sm text-text-primary font-mono">{selectedTransaction.connection_id || 'N/A'}</dd>
                        </div>
                        <div>
                          <dt className="text-sm font-medium text-gray-500">Created At</dt>
                          <dd className="text-sm text-text-primary">{formatDate(selectedTransaction.created_at)}</dd>
                        </div>
                        <div>
                          <dt className="text-sm font-medium text-gray-500">Updated At</dt>
                          <dd className="text-sm text-text-primary">{formatDate(selectedTransaction.updated_at)}</dd>
                        </div>
                      </dl>
                    </div>
                  </div>
